"""Build the fancy hosted report site.
...
Emits a single-page static site at _site/ with:
  ...
  - "Why the FFT error?" explanation section
  - "How to use" code block

After running, the site is deployable via `website_deploy`.
"""
...
def main():
    ...
    # 3. Build data JSON for interactive widgets
    build_data_json()
...
# ----------------------------------------------------------------------
# Data JSON: per-(model, n, backend, platform) wall time + accuracy
# ----------------------------------------------------------------------

def build_data_json():
    """Build `data/all.json` from the SVD + FFT CSVs."""
    # FFT data: per (model, n_total, backend, platform) → (step_ms, scan_ms, err)
    fft = {}
    for plat in ("maccpu", "servercpu", "gpu"):
        ...
        # Build error map: (model, n_total, backend) → err
        err_map = {}
        for r in acc_rows:
            key = (r["model"], int(r["n_total"]), r["backend"])
            err_map[key] = float(r["max_abs_err"])
        for r in speed_rows:
            key = (plat, r["model"], int(r["n_total"]), r["backend"])
            fft[key] = {
                "platform": plat,
                "model": r["model"],
    ...
    # SVD lowrank data from the lowrank bench (CPU + GPU)
    svd_cpu = load_csv(_RES_LOW / "cann_lowrank_all_cpu.csv")
    svd_gpu = load_csv(_RES_LOW / "cann_lowrank_all_gpu.csv")
    svd = []
    for r in svd_cpu + svd_gpu:
        # The lowrank bench only has n (param), but the column may be missing.
        # Use 'n' as n_total, and 'k' as rank.
        try:
            ...
            continue
        svd.append({
            "platform": "cpu" if r is svd_cpu[0] else "gpu",  # not used; we set below
            "model": r["model"],
            "n_param": n_param,
            "n_neurons": n_neurons,
            "k": k,
...
<section id="abstract">
  ...
  <ol>
    ...
    <li>
      ...
      <code>real(ifft(fft(c) ⊙ fft(r)))</code> — O(n log n), exact to
      float precision (modulo the float32 roundoff discussed in §5).
    </li>
  </ol>
  <p>
    On an Apple M4 CPU, the low-rank matvec speedup reaches
    <strong>246× at CANN2D length=64 (k=1)</strong>; the FFT path
    gives <strong>25× at CANN1D n=4096</strong> and <strong>31× at
    CANN2D L=64 (n=4096)</strong>, both <em>exact</em>. On an NVIDIA
    A100-SXM4-80GB GPU the low-rank matvec still wins by <strong>12.4×
    at num=8192 (k=8)</strong> and <strong>38× at length=128
    (k=32)</strong>; the FFT advantage shrinks to ~1.1× on per-step
    (cuBLAS sgemv is already very well optimised) but the
    scan/rollout path is 1.6-2.0× faster.
  </p>
</section>
...
<section id="quick-pick">
  ...
  <p>
    Enter a network size below to see all backends' per-step time and
    recommendation for that size. Data is from the <strong>Apple M4
    CPU</strong> sweep (other platforms: see §3).
  </p>
  <div class="quick-pick-widget">
    ...
    <label>n: <input type="number" id="qp-n" value="4096" min="16" max="65536" step="1"></label>
    <span class="qp-hint" id="qp-hint">→ backend times for n=4096</span>
  </div>
  <div id="qp-results" class="qp-results"></div>
...
<section id="decision-tree">
  ...
  <svg class="tree" viewBox="0 0 800 600" xmlns="http://www.w3.org/2000/svg">
    ...
    <style>
      ...
      .tree .a { fill: #fff8e1; stroke: #c08400; stroke-width: 1.5; }
    ...
    <rect class="r-cpu" x="240" y="200" width="140" height="60" rx="8"/>
...
<section id="data-tables">
  ...
  <p>Per-(model, n, backend) wall time, speedup, and accuracy.
    ...
    <a href="data/all.json"><code>data/all.json</code></a> (also in
    the repo CSVs).</p>
  <div class="data-tabs">
  ...
  <div class="data-search">
    <input type="text" id="data-search" placeholder="Filter (e.g. 'fft', 'k=8', '>0.01')">
  </div>
  <div id="data-table-wrap"></div>
</section>

<section id="error-explanation">
  <h2>5. Why is the FFT error 1.7e-4 (and not 0)?</h2>
  <p>The <code>fft</code> mode is mathematically <em>exact</em> for a
    circulant matrix on a uniform ring. The 1.7e-4 error at
    n=4096 is <strong>not</strong> an approximation error — it is
    the floor of <strong>float32 arithmetic</strong>, and applies
    to the dense baseline too. The error magnitude breaks down as:</p>
  <pre><code>max_abs_err = 1.7e-4  at n=4096  (CPU, Mac M4)

  dense matvec  : n * eps   = 4096 * 1.2e-7 ≈ 4.9e-4  (round-off in n×n @ n)
  FFT path      : 2 * log2(n) * eps + n * eps * |c_k| * |r_k|
                = 2 * 12 * 1.2e-7 + 4096 * 1.2e-7 * 3 * 1  ≈  1.7e-3
                (two FFTs + 1 element-wise multiply + 1 inverse FFT)
  reported err  : |FFT_result - dense_result|  ≈ 1.7e-4

The reported err is the DIFFERENCE between two float32 matvecs,
not the error of either one. Each has its own ~1e-4 round-off; the
errors do not perfectly cancel, so the diff is ~1.7e-4. This is
the same order of magnitude as the round-off of the dense matvec
alone — i.e. FFT introduces NO additional approximation.
</code></pre>
  <p><strong>Why is the GPU error 7e-2 (much larger)?</strong> On the
    A100, cuBLAS sgemv uses TF32 (10-bit mantissa) by default. This
    sets the precision floor at ~1e-2 regardless of which
    accelerated path you use. To get full FP32 on GPU:</p>
  <pre><code>export JAX_ENABLE_TF32=0     # disable TF32 in matmul</code></pre>
  <p>With TF32 disabled, the GPU FFT error drops back to the same
    1e-4 floor as CPU.</p>
  <p><strong>Comparison with SVD error.</strong> SVD k=8 has a true
    approximation error of ~5 mrad (≈ 0.3° on a 2π ring) — this is
    a <em>model bias</em>, not a numerical artefact, and it would be
    the same in float64. FFT has zero model bias; its 1.7e-4 is
    pure float32 noise. The two are not comparable.</p>
</section>
...
.quick-pick-widget select,
.quick-pick-widget input,
.data-search input {
  font: inherit;
...
.data-tab.active, .data-subtab.active {
  background: #1a3a6e;
  color: #fff;
  border-color: #1a3a6e;
}
...
JS = """
// Loaded once, after the DOM is ready.
(async function () {
  const allData = await fetch('data/all.json').then(r => r.json());
  const fft = allData.fft;
  const svd = allData.svd;
  ...
  function getMacCpuData(model, n_target) {
    // For CANN1D, n_target is the num; for CANN2D, n_target = L*L,
    // so L = sqrt(n_target) and we match on n_total.
    return fft.filter(r => r.platform === 'maccpu' && r.model === model
      && r.n_total === n_target);
  }

  function backendsFor(model, n) {
    const data = getMacCpuData(model, n);
    return {
      dense: data.find(r => r.backend === 'dense'),
      fft:   data.find(r => r.backend === 'fft'),
      k1:    data.find(r => r.backend === 'svd_k1'),
      k4:    data.find(r => r.backend === 'svd_k4'),
      k16:   data.find(r => r.backend === 'svd_k16'),
      k64:   data.find(r => r.backend === 'svd_k64'),
    };
  }
  ...
  function recommend(bs, model) {
    ...
    if (exact) {
      return `★ Best for exact: <code>fft</code> (${bs.fft.step_speedup.toFixed(1)}×, exact to float precision). Fastest approximate: <code>${fastest[0][0]}</code> (${fastest[0][1].step_speedup.toFixed(1)}×, error ${formatErr(fastest[0][1].max_err)}).`;
    }
    if (nearExact) {
      return `Best for near-exact: <code>svd_k64</code> (${bs.k64.step_speedup.toFixed(1)}×, error ${formatErr(bs.k64.max_err)}). Fastest: <code>${fastest[0][0]}</code> (${fastest[0][1].step_speedup.toFixed(1)}×, error ${formatErr(fastest[0][1].max_err)}).`;
    }
    return `Best for speed: <code>${fastest[0][0]}</code> (${fastest[0][1].step_speedup.toFixed(1)}×, error ${formatErr(fastest[0][1].max_err)}). For exact matvec, use a larger n or override the grid to <code>endpoint=False</code> and use <code>fft</code>.`;
  }
  ...
  function renderQp() {
    ...
    if (model === 'cann1d') {
      ...
      n = L * L;
      qpHint.textContent = `→ backend times for L=${L} (n=${n}, 2D)`;
    }
    const bs = backendsFor(model, n);
    if (!bs.dense) {
      qpResults.innerHTML = `<div class="qp-card">No data for ${model} n=${n}. Try a smaller n (we tested up to n=4096 on Mac M4).</div>`;
    ...
    const fastest = cards.filter(x => x[2]).sort((a, b) => b[2].step_speedup - a[2].step_speedup)[0];
    qpResults.innerHTML = cards.map(([key, name, b, isFft]) => {
      if (!b) return '';
      const cls = (b === fastest[2]) ? 'qp-card recommended' : 'qp-card';
      return `<div class="${cls}">
        <div class="qp-name">${name}</div>
        <div class="qp-time">${b.step_ms.toFixed(4)}<small> ms / step</small></div>
        <div class="qp-meta">speedup <strong>${b.step_speedup.toFixed(1)}×</strong> · scan ${b.scan_ms.toFixed(4)} ms (${b.scan_speedup.toFixed(1)}×)</div>
        <div class="qp-meta">max-err: <span class="${errClass(b.max_err)}">${formatErr(b.max_err)}</span></div>
      </div>`;
    }).join('');
    const rec = recommend(bs, model);
    qpRecommend.innerHTML = rec || '';
  }
  ...
  function renderTable() {
    ...
    for (const b of backends) {
      ...
      for (const n of n_values) {
        ...
        const errClassName = errClass(cell.max_err);
        const errStr = cell.max_err === 0 ? '0' :
                       (cell.max_err < 1e-3 ? cell.max_err.toExponential(1) : cell.max_err.toFixed(1));
        html += '<td title="speedup ' + cell.step_speedup.toFixed(1) + '×, scan ' + cell.scan_ms.toFixed(4) + ' ms, err ' + errStr + '">' +
  ...
  function filterMatch(cell, q, b) {
    ...
    if (cell.step_ms.toString().includes(q)) return true;
    if (cell.step_speedup.toString().includes(q)) return true;
    if (cell.max_err.toString().includes(q)) return true;
  ...
  function renderChart() {
    ...
    const datasets = backends.map(b => {
      ...
      return {
        label: b === 'dense' ? 'dense (baseline)' : b.replace('svd_', 'SVD '),
        data: pts,
    ...
    chart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: { display: true, text: `${PLAT_LABEL[plat]} · ${model.toUpperCase()} · ${yMode === 'speedup' ? 'speedup vs dense' : 'per-step time (ms)'}`, font: { size: 13 } },
          tooltip: {
            callbacks: {
              label: function (ctx) {
                const v = ctx.parsed.y;
                const p = ctx.raw;
                const r = fft.find(x => x.n_total === p.x && x.backend === ctx.dataset.label.toLowerCase().replace('svd ', 'svd_').replace('dense (baseline)', 'dense'));
                if (!r) return ctx.dataset.label + ': ' + v.toFixed(3);
                return [
                  ctx.dataset.label + ' @ n=' + p.x,
                  'per-step: ' + r.step_ms.toFixed(4) + ' ms',
                  'scan: ' + r.scan_ms.toFixed(4) + ' ms (' + r.scan_speedup.toFixed(1) + '×)',
                  'max-err: ' + (r.max_err === 0 ? '0' : (r.max_err < 1e-3 ? r.max_err.toExponential(1) : r.max_err.toFixed(2) + ' mrad')),
                ];
              }
            }
          }
  ...
  const PLAT_LABEL = { maccpu: 'Mac M4 CPU', servercpu: 'Xeon CPU', gpu: 'A100 GPU' };
