...
def render_markdown(
    ...
    md.append(
        ...
        "we measure (i) per-step time of the recurrent matvec in "
        "isolation (via a `lax.scan` of 200 matvecs), (ii) per-step "
        "time of the full update step, and (iii) the bump-tracking "
        "error of the network under a slow moving-stimulus trajectory. "
        "On a single Apple M3 Pro CPU core, the matvec speedup reaches "
        "**245× at CANN1D num=4096 (k=8)** and **223× at CANN2D "
        "length=64 (k=8)**, with the bump-position error staying below "
        "5 mrad (≈ 0.3° on a 2π ring). On an NVIDIA A100-SXM4-80GB GPU "
        "the matvec speedup is launch-bound: CANN1D crosses at n ≈ 4096 "
        "(3.3×) and reaches **12.4× at num=8192 (k=8)**, while CANN2D "
        "crosses much earlier and reaches **38× at length=128 (k=32)** "
        "thanks to the larger dense-matvec baseline. The accuracy "
        "numbers are independent of the hardware — they are a property "
        "of the low-rank factorisation.\n"
    )
    md.append(
