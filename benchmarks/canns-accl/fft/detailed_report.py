...
def build_summary(platforms: dict[str, list[dict]], accuracy: dict[str, list[dict]]) -> str:
    md = []
    ...
    for plat_key in ("maccpu", "servercpu", "gpu"):
        ...
        for model in ("cann1d", "cann2d"):
            ...
            if not n_params:
                continue
            backends_in_data = [
                b for b in ACCURACY_ORDER if any(b in by_cell[(model, n)] for n in n_params)
            ]
            # Header
            hdr = (
                ["n"]
                + [f"{BACKEND_INFO[b]['label']} step (ms)" for b in backends_in_data]
                + [f"{BACKEND_INFO[b]['label']} scan (ms)" for b in backends_in_data]
                + [f"{BACKEND_INFO[b]['label']} err" for b in backends_in_data]
            )
            md.append("| " + " | ".join(hdr) + " |")
            ...
            for n_p in n_params:
                ...
                for b in backends_in_data:
                    err = float(
                        acc_by_cell[(model, n_p)].get(b, {}).get("max_abs_err", float("nan"))
                    )
            ...
            for n_p in n_params:
                cells = [f"n={n_p}"]
                base_t = float(
                    by_cell[(model, n_p)].get("dense", {}).get("per_step_ms", float("nan"))
                )
                for b in backends_in_data:
    ...
    md.append("```\n")
    md.append(
        '**Take-away**: `fft` and `svd_k64` cluster at "exact, ~25× faster". '
    ...
    md.append("```\n")
    md.append(
        "Note: there's a **gap between 1 mrad and 29 mrad** — `svd_k16` jumps from "
    ...
    md.append("```\n")
    md.append(
        "**Insight**: dropping accuracy from 1 mrad to 30 mrad (1.7e-4 → 29) "
...
def fig_tradeoff(
    ...
    fig, axes = plt.subplots(len(platform_order), 2, figsize=(7.5, 7.5), sharex=False, sharey=False)
    for pi, plat_key in enumerate(platform_order):
        ...
        for ci, (m, model_title) in enumerate([("cann1d", "1D CANN"), ("cann2d", "2D CANN")]):
            ...
            for r in acc_rows:
                if r["model"] == m:
                    by_n.setdefault(int(r["n_param"]), {}).setdefault("__acc__", {})[
                        r["backend"]
                    ] = float(r["max_abs_err"])
            if not by_n:
                ax.set_axis_off()
    ...
    fig.suptitle(
        f"Speed vs accuracy trade-off — all backends × all platforms {title_suffix}", y=1.00
    )
    fig.tight_layout()
        ...
        for bi, backend in enumerate(backends_to_plot):
            ...
            for s in sizes:
                ...
                row_data = [
                    r
                    for r in rows
                    if r["model"] == m and int(r["n_param"]) == s and r["backend"] == backend
                ]
                if not row_data:
                    ys.append(0)
                    continue
            ...
            ax.bar(
                ...
                ys,
                width,
                color=BACKEND_INFO[backend]["color"],
                label=BACKEND_INFO[backend]["label"],
                alpha=0.85,
            )
        ax.set_xticks(x)
