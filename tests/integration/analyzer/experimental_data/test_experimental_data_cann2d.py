...
def test_decode_circular_coordinates():
    ...
    embed_spikes, *_ = embed_spike_trains(
        embed_data, config=SpikeEmbeddingConfig(smooth=True, speed_filter=False)
    )
...
def test_array_indexing_fix():
    ...
    # Mock cocycle data
    cocycle = np.array([[0, 1, 5], [1, 2, 3], [2, 3, 7], [3, 4, 2], [4, 0, 1]])
...
def test_plot_3d_bump_on_torus():
    ...
    decoding_result = {"coordsbox": coords, "times_box": times}
    ...
    try:
        ...
        ani = plot_3d_bump_on_torus(
            decoding_result=decoding_result, spike_data=embed_data, config=config
        )
