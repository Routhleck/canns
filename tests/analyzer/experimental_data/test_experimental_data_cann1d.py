...
def test_create_1d_bump_animation():
    ...
    try:
        ...
        config = CANN1DPlotConfig.for_bump_animation(
            fps=5, show=False, figsize=(6, 4), max_height_value=0.6, nframes=n_steps
        )
        ...
        animation_old = create_1d_bump_animation(
            fitting_results, save_path=None, fps=5, show=False, figsize=(6, 4)
        )
...
def test_create_1d_bump_animation_with_saving():
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            animation = create_1d_bump_animation(fitting_results, config=config)
