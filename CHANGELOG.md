...
### Added
- **Resilient HuggingFace dataset downloads (`HF_ENDPOINT` + automatic mirror fallback, #101).** `canns-team/data-analysis-datasets` downloads now honour the standard `HF_ENDPOINT` env var (set it to `https://hf-mirror.com` to switch hosts) and automatically retry the same file from `hf-mirror.com` if the primary URL fails (timeout / DNS / 5xx). The progress bar is preserved across the fallback, and the success log records which endpoint served the file. If all endpoints fail, the error message is actionable: it tells the user how to drop the `.npz` into the local cache directory and load it offline via `load_grid_data(dataset_key=...)`. Verified end-to-end (8.7 MB `grid_1.npz` downloads in ~6 s via mirror fallback where the primary URL was timing out).
- **`accl_mode="fft"` for exact circulant matvec on CANN1D, CANN2D.** A fifth acceleration mode that exploits the circulant structure of the Gaussian distance kernel on a uniform ring (1D) or torus (2D). The matvec is computed exactly via `real(ifft(fft(c) ⊙ fft(r)))` in O(n log n) instead of O(n²). On a clean circulant (`endpoint=False` uniform grid) the FFT path is **exact** (rel err < 1e-5 at float precision, independent of n) and gives 25-50× per-step speedup on CPU at `n=4096`. On A100 GPU the per-step win is small (~10%) because cuBLAS sgemv is already very well-optimized; the scan/rollout path still benefits (1.6-2.0×). The canns default `endpoint=True` grid is not circulant under the canns wrap convention, so the FFT path silently falls back to `"normal"` with a `UserWarning` on that grid. To get the FFT speedup, override the grid:
    ...
    model.set_accl_mode("fft")
    # CANN2D: also override model.y and rebuild.
    ```
    `ACCL_MODES` now exports `("normal", "fast", "ultra-fast", "auto", "fft")`. See `benchmarks/cann_fft/` for the CPU+GPU benchmark, `benchmarks/cann_fft/results/cann_fft_summary.md` for the full writeup, and `examples/cann/cann1d_fft_mode.py` for a runnable demo.

- **`accl_mode` / `accl_k` constructor args on CANN1D, CANN2D, CANN1D_SFA, CANN2D_SFA.** Both 1D and 2D models (and their spike-frequency-adaptation variants) now accept `accl_mode="normal" | "fast" | "ultra-fast"` and an optional explicit `accl_k`. The recurrent matvec `Irec = conn @ r` is replaced by a low-rank truncated-SVD factorisation `Irec = U_l @ (V_l.T @ r)` whenever `accl_mode != "normal"`. The default is `"normal"` (full rank, identical to the pre-existing behaviour). Preset ranks are `8` (CANN1D `fast`), `1` (CANN1D `ultra-fast`), `32` (CANN2D `fast`), and `4` (CANN2D `ultra-fast`) — these are the smallest ranks that keep the moving-bump position error below ~5 mrad per the `benchmarks/cann_lowrank/` benchmark. A `set_accl_mode(mode=None, k=None)` method switches the mode at runtime. Bump-position error is at most ~5 mrad (≈ 0.3° on a 2π ring) at the recommended `fast` defaults; the in-model full-step speedup grows with `n` (≈ 1.2× at CANN2D `length=64`, ≈ 5× when the same n=4096 model is run with a tighter timing harness). See `benchmarks/cann_lowrank/results/cann_lowrank_summary.md` for the full speed/accuracy sweep and `examples/cann/cann1d_accl_modes.py` for a runnable demo.
- **`accl_mode="auto"` + `accl_target_err_mrad` on CANN1D, CANN2D, CANN1D_SFA, CANN2D_SFA.** A fourth preset that picks the smallest rank from the SVD spectrum of `conn_mat` such that the predicted bump-position error is at most `accl_target_err_mrad` (milliradians, default `5.0`). The heuristic is calibrated against the `benchmarks/cann_lowrank/` sweep: `target >= 4.0` → `k=1`; `target >= 1.0` → smallest `k` with ≥ 50% energy captured; `target >= 0.5` → ≥ 90% energy; `target >= 0.1` → ≥ 99% energy; `target < 0.1` falls back to dense (no low-rank can satisfy). The spectrum is computed once at `__init__`, so the per-step cost is just the two small GEMVs. The same knob is exposed at runtime via `set_accl_mode("auto", target_err_mrad=…)` (the budget is remembered across calls). For the default 5.0 mrad budget the auto mode selects `k=1` for both 1D and 2D (because the leading singular vector of a Gaussian kernel *is* the bump shape); for `target=0.5` it selects the same rank as the existing `fast` preset, so `auto` is a strict superset that lets users dial in any intermediate speed-vs-accuracy point.
- **`_pick_k_for_err_target(spectrum, target_err_mrad)` exported helper** in `canns.models.basic.cann`. Pure-function utility: given the singular values of a `conn_mat` and a milliradian budget, returns the smallest `k` that satisfies the heuristic (or `-1` for "use dense"). The same calibration table as the `auto` mode uses; useful for users who want to pick `k` for a custom `accl_k` outside the four presets.
- **`ACCL_MODES`, `ACCL_DEFAULT_K`, and `_pick_k_for_err_target` exported from `canns.models.basic.cann`.** Use these to discover the available modes, their default ranks, and the spectrum-based auto-pick heuristic without hard-coding them.
- **`benchmarks/cann_lowrank/` benchmark and `REVIEW.md` correctness audit.** Self-contained sweep over `CANN1D num ∈ {64, 128, 256, 512, 1024, 2048, 3072, 4096}` (CPU) / `{64 … 8192}` (GPU) and `CANN2D length ∈ {8, 16, 32, 48, 64}` (CPU) / `{8 … 128}` (GPU) with rank `k ∈ {1, 2, 4, 8, 16, 32, 64, 128, 256}`. Reports per-step time (matvec-only via `lax.scan` of 200 matvecs and full-step), bump-position error (mrad), and `r_max` error. CSV + markdown + NeurIPS-styled HTML + PDF writeup. The audit doc covers SVD-factor correctness, symmetric-`conn_mat` argument, stimulus and bump-position math, error-metric definitions, and explicit caveats. Optional `--long-trajectory` flag adds a T=2000 slow-sweep drift test (`bump_drift_{tag}.npz` + `fig_long_drift_{1d,2d}.png`); §3.7 of the report covers the result: the low-rank drift is *bounded* for every `k`, sub-mrad at the recommended `k=8` (1D) / `k=32` (2D).
...
### Added
- New Trainer infrastructure with base `Trainer` class, callback system, early stopping, and LR schedulers
  - `src/canns/trainer/_base.py`: Abstract Trainer base with `fit()` method
  - `src/canns/trainer/_callback.py`: Callback, CallbackContext, CallbackList
  - `src/canns/trainer/_early_stopping.py`: EarlyStopping support
  - `src/canns/trainer/_lr_scheduler.py`: CosineAnnealing, StepDecay, WarmupDecay
  - `src/canns/trainer/_history.py`: History tracking
- `canns.utils.example_outputs` module for unified example output directory handling

### Changed
- Updated Hopfield capacity formula from N/(4·ln(N)) to 0.14·N (classical theoretical estimate)
...
- Removed "placeholder image" notes from ASA GUI documentation
- Updated research impact statement to reflect broader usage
- Unified example output directories across all example scripts (PR #97)

### Fixed
- Removed unused numpy import from hopfield.py
- Adjusted Hopfield capacity test expectation to match 0.14·N formula
