...
本文档阐述了 [ANONYMOUS_PROJECT] 库的核心设计原则和模块组织结构。

核心设计原则
======================

[ANONYMOUS_PROJECT] 库基于两个基本原则构建，这些原则指导着其架构和实现。
...
:🏗️ **模型** (``[ANONYMOUS_PROJECT].models``)：
   定义神经网络动力学和状态演化

:📊 **任务** (``[ANONYMOUS_PROJECT].task``)：
   生成实验范式和输入数据

:📈 **分析器** (``[ANONYMOUS_PROJECT].analyzer``)：
   可视化和分析仿真结果

:🧠 **训练器** (``[ANONYMOUS_PROJECT].trainer``)：
   为脑启发模型实现学习规则

:🔗 **流水线** (``[ANONYMOUS_PROJECT].pipeline``)：
   编排完整的实验工作流
...
* ``[ANONYMOUS_PROJECT].models.basic.BasicModel`` 用于基本 CANN 模型
* ``[ANONYMOUS_PROJECT].models.brain_inspired.BrainInspiredModel`` 用于脑启发模型
* ``[ANONYMOUS_PROJECT].trainer.Trainer`` 用于训练算法
...
[ANONYMOUS_PROJECT] 库支持四种不同的工作流，每种工作流针对不同的研究需求。这些场景展示了架构的模块化设计和灵活性。

.. figure:: ../../_static/[ANONYMOUS_PROJECT]_scenarios_custom.png
   :align: center
   :width: 100%

   [ANONYMOUS_PROJECT] 四个核心应用场景
...
.. grid:: 2
   ...
   .. grid-item-card:: 🔗 场景 4：端到端流水线
      ...
      **步骤：**
      ...
      5. **输入配置**：通过配置文件指定参数
      6. **流水线编排**：``[ANONYMOUS_PROJECT].pipeline`` 协调所有模块
      7. **自动执行**：自动创建模型、仿真和分析
      8. **输出报告**：生成综合结果摘要
...
[ANONYMOUS_PROJECT] 库基于 BrainPy :cite:p:`wang2023brainpy`（``brainpy``）构建，这是一个强大的脑动力学编程框架。BrainPy 提供：
...
``[ANONYMOUS_PROJECT].pipeline`` 模块将所有其他模块协调成完整的实验工作流。它管理从模型设置到任务执行再到结果分析的完整周期，为常见用例提供高级接口。
...
原生层（[ANONYMOUS_PROJECT]-lib）
~~~~~~~~~~~~~~~~~~~~~~~~~

.. important::

   对于 Python 开销显著的性能关键操作，该库通过 ``[ANONYMOUS_PROJECT]-lib`` 提供可选的 Rust 驱动后端：
   ...
   * **未来模块**：计划支持近似最近邻、动力学计算
   ...
   [ANONYMOUS_PROJECT]-lib 集成遵循相同的原则：它提供简单的 Python API，同时利用原生性能处理瓶颈操作。用户可以选择使用这些加速功能而无需更改代码结构。
...
.. note::
   ...
   训练器继承自 ``[ANONYMOUS_PROJECT].trainer.Trainer`` 并实现：
...
[ANONYMOUS_PROJECT] 库通过精心的架构选择实现其目标：
