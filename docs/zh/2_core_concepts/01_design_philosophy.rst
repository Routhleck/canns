...
CANNs 库围绕两个基本原则构建，这些原则指导着其架构和实现。
...
.. grid:: 2
   ...
   .. grid-item-card:: 📊 场景 2：数据分析
      ...
      **步骤：**
      ...
      3. **吸引子/动力学分析**：应用凸起拟合、解码、拓扑数据分析方法
...
CANNs 库建立在 BrainPy :cite:p:`wang2023brainpy`（``brainpy``）之上，这是一个强大的脑动力学编程框架。BrainPy 提供：
...
这种平衡通过合理的默认值和丰富的自定义选项来实现。例如，``CANN1D()`` 使用适用于大多数情况的默认参数，但每个参数都可以显式指定。
...
.. important::
   ...
   * **未来模块**：计划支持近似最近邻、动力学计算
   ...
   canns-lib 集成遵循相同的原则：暴露简单的 Python API，同时利用原生性能处理瓶颈操作。用户可以选择使用这些加速功能而无需更改代码结构。
