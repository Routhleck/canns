...
本文档解释了[ANONYMOUS_PROJECT]库中的脑启发学习机制和Trainer框架。

概览
========

训练器模块（``[ANONYMOUS_PROJECT].trainer``）提供了统一的接口，用于使用生物学合理的学习规则训练脑启发模型。与使用反向传播的传统深度学习不同，这些方法依赖于局部、活动依赖的可塑性。
...
要创建新训练器，从 ``[ANONYMOUS_PROJECT].trainer.Trainer`` 继承并实现：
