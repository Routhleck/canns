...
``[ANONYMOUS_PROJECT]-experiments/figure2`` 中所有子图对应的可视化结果，并以 PNG 输出。
...
- 已安装 [ANONYMOUS_PROJECT]（建议 ``pip install -e .`` 或 ``pip install [ANONYMOUS_PROJECT]``）
- 终端宽度建议不低于 120 列，至少 100 列

启动 Model Gallery TUI
----------------------

推荐方式：使用统一入口 ``[ANONYMOUS_PROJECT]-tui``，启动后选择 **Model Gallery**。
...
Gallery 的分析项与 ``[ANONYMOUS_PROJECT]-experiments/figure2`` 对齐（见 :ref:`全结果合成图 <fig-gallery-full>`）：
...
1. 启动 ``[ANONYMOUS_PROJECT]-tui``，在选择页进入 **Model Gallery**。\n
