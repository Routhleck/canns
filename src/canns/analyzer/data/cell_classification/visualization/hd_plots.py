...
def plot_polar_tuning(
    ...
    if preferred_direction is not None:
        ...
        ax.plot([preferred_direction, preferred_direction], [0, max_rate],
               'r--', linewidth=2, label=f'Preferred: {np.rad2deg(preferred_direction):.1f}°')
...
def plot_hd_analysis(
    ...
    head_directions: Optional[np.ndarray] = None,
    ...
    if result.preferred_direction is not None:
        ax2.axvline(np.rad2deg(result.preferred_direction), color='r',
                   linestyle='--', linewidth=2, label='Preferred direction')
