...
def load_roi_data(source: Optional[Union[str, Path]] = None) -> Optional[np.ndarray]:
    """
...
def load_grid_data(source: Optional[Union[str, Path]] = None, 
                   dataset_key: str = "grid_1") -> Optional[Dict[str, Any]]:
    """
    ...
    for pos_key in ['x', 'y']:
        if pos_key in data:
            ...
            if pos_data.shape != t_data.shape:
                print(f"Position data '{pos_key}' shape {pos_data.shape} doesn't match time data shape {t_data.shape}")
                return False
    
    return True


def preprocess_spike_data(spike_data: Union[list, np.ndarray],
                         time_window: Optional[Tuple[float, float]] = None,
                         min_spike_count: int = 10) -> Optional[np.ndarray]:
    """
...
def get_data_summary(data: Union[np.ndarray, Dict[str, Any]]) -> Dict[str, Any]:
    """
