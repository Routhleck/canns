"""CANN1D acceleration modes (``accl_mode``).

The recurrent matvec ``Irec = conn @ r`` can be replaced by a
low-rank SVD factorisation (4 modes) or by an exact circulant
FFT (the 5th mode). This example drives a ``CANN1D`` with
``SmoothTracking1D`` and prints the rank each mode picks, then
shows the FFT-mode setup on a clean grid.
...
def main():
    ...
    task = SmoothTracking1D(
        cann_instance=cann,
        Iext=(0.0, np.pi / 2, np.pi, 3 * np.pi / 2, 2 * np.pi),
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        cann_default = CANN1D(num=128, accl_mode="fft")
    print(f"  default grid: requested=fft, got={cann_default.accl_mode}  (fallback)")
    print(f"  warning: {str(caught[0].message)[:80]}...")

    # 2) clean grid -> FFT path activated, exact to fp32.
    cann_clean = CANN1D(num=128, accl_mode="normal")
    cann_clean.x = bm.linspace(-bm.pi, bm.pi, 128, endpoint=False)
